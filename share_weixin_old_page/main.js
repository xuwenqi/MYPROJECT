...
var tool = {
    ...
    initVueModel: function(curObj) {
        if (curObj.videoIndex != -1) {
            ...
            console.log(curObj)
            var obj = '<div></div>';
            var curName = $(obj).html(curObj.videoList[curObj.videoIndex].user_name).text();
            var userInfo = curObj.videoList[curObj.videoIndex].user;
            $('.avatar').attr('src', curObj.videoList[curObj.videoIndex].user_img);
            ...
            $('.video-opt').attr('src', curObj.videoList[curObj.videoIndex].video_url);
            $('.video-opt').attr('poster', curObj.videoList[curObj.videoIndex].cover);
            $('.bg-video img').attr('src', curObj.videoList[curObj.videoIndex].cover);
        }
        if (curObj.videoIndex == 0) {
            ...
            $('#share-pic').attr('src', curObj.videoList[0].video_url);
        }

    },
    getRecomList: function() {
        var self = this;
        var url = '/inc/moment/share/getRecommendMoments';
        $.ajax({
            useCache: 1,
            url: url,
            type: 'GET',
    ...
    getPersonInfo: function() {
        var _this = this;
        var url = '/inc/moment/share/getMoments';
        $.ajax({
            useCache: 1,
            url: url,
            type: 'POST',
            dataType: 'json',
            data: { momentids: args.momentids || '' },
